...
def ajusta_colunas(df):
    ...
    df['DATA FINAL'] = pd.to_datetime(df_main['DATA FINAL'])
    df['DATA MEDIA'] = ((df['DATA FINAL'] - df['DATA INICIAL'])/2) + df['DATA INICIAL']
    df = df_main.sort_values(by='DATA MEDIA',ascending=True)
...
def filtra_df(df, filter):
    return df_main[df_main.PRODUTO == filter]

df_main = read_csv("data/data.csv")
df_main = filtra_df(df_main, 'GASOLINA COMUM')
df_main = ajusta_colunas(df_main)



df_store = df_main.to_dict() # para salvar no dcc.store
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    ...
                    dbc.Row([
                        dbc.Button("Visite o Site", href="https://github.com/evandroviero/dashboards", target="_blank")
                    ], style={'margin-top': '10px'})
                ])
            ], style=tab_card)
        ], sm=4, lg=2),
